...
app.post(
  ...
  [
    ...
    check("name")
      .trim()
      .isLength({ min: 5 })
      .withMessage("namee must be of atleast 5 characters"),
    check("email").notEmpty().withMessage("Please Enter the Email"),
    check("email", "The email must be valid").isEmail().normalizeEmail(),
    check("phone", "The phone number must be numeric").isNumeric(),
    check("hobbie", "Please Do Select one hobby").notEmpty(),
    check("gender", "Plesase Do Select your Gender").notEmpty(),
  ],
  (req, res) => {
